...
def getmessage(id):
    ...
    :return: int
    """
    use = Messages.objects.get(id=id)
    return use
...
def chat(request, username):
    ...
    if request.method == "GET":
        ...
        return render(request, "chat/messages.html",
                      {'messages': messages,
                       'friends': friends,
                       'curr_user': curr_user, 'friend': friend,'songs':songs})
...
def add_song(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/')  # Redirect to a page displaying the list of songs
    else:
        form = song_form()
    
    
    return render(request, 'chat/add_song.html', {'form': form})


def add_song_message(request,id,song_id):
    message = getmessage(id)
    song = getsong(song_id)
    message.song_id = song
    message.save()
    
    return render(request,"chat/t.html")
    ...
    if request.method == "POST":
        ...
        for song in songs:
            if query.lower() in song.title.lower():
                song_ls.append(song)
        return render(request, "chat/choose_song.html", {'songs': song_ls ,'id':id})

    try:
        songs = songs[:10]
    except:
        songs = songs[:]
    context={'songs': songs,'id':id}
    #  {'songs': songs},{'id':id}
    return render(request, "chat/choose_song.html",context)
